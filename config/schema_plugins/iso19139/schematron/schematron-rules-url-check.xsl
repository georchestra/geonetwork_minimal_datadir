...
<xsl:template match="/" mode="generate-id-2">U</xsl:template>
   ...
   <!--SCHEMA SETUP-->
<xsl:template match="/">
      <svrl:schematron-output xmlns:svrl="http://purl.oclc.org/dsdl/svrl" title="URL checks" schemaVersion="">
         <xsl:comment>
   ...
   <!--SCHEMATRON PATTERNS-->
<svrl:text xmlns:svrl="http://purl.oclc.org/dsdl/svrl">URL checks</svrl:text>
...
<svrl:text xmlns:svrl="http://purl.oclc.org/dsdl/svrl">
  ...
  <!--RULE
      -->
<xsl:template match="//gmd:linkage//gmd:URL[starts-with(text(), 'http')]" priority="1000"
                 mode="M7">
      <svrl:fired-rule xmlns:svrl="http://purl.oclc.org/dsdl/svrl"
                       context="//gmd:linkage//gmd:URL[starts-with(text(), 'http')]"/>
      <xsl:variable name="status" select="xslutil:getURLStatusAsString(text())"/>
